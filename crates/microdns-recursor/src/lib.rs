...
impl RecursorServer {
    ...
    /// Create a recursor that reads forward zones from the database on each query.
    /// The static forward table is empty — all forwarding comes from DB.
    pub fn from_db(listen_addr: SocketAddr, db: Db, cache_size: usize) -> anyhow::Result<Self> {
        let cache = Arc::new(DnsCache::new(cache_size));
        // Empty static table — resolver reads forwarders from DB directly
        let forward_table = Arc::new(ForwardTable::from_config(&Default::default()));

        let resolver = Arc::new(Resolver::new(cache, forward_table, Some(db)));
